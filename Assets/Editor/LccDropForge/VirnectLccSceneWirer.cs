...
namespace LccDropForge
{
    internal static class VirnectLccSceneWirer
    {
        ...
        [MenuItem("Tools/Lcc Drop Forge/Dev · Build Scene4 — A/B Compare (Virnect | Aras-P, 1st_Cutter)")]
        static void DevBuildScene4ABCompare()
        {
            const string scenePath = "Assets/Scenes/Scene4_AB_Compare.unity";
            const string lccPath   = "Assets/LCC_Drops/ShinWon_1st_Cutter/ShinWon_1st_Cutter.lcc";
            const string asetPath  = "Assets/GaussianAssets/ShinWon_1st_Cutter_lod0.asset";

            // 0) 안전: 현재 씬이 dirty 면 사용자에게 저장 묻기
            var currentScene = UnityEditor.SceneManagement.EditorSceneManager.GetActiveScene();
            if (currentScene.isDirty)
            {
                if (!UnityEditor.SceneManagement.EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo())
                { Debug.LogWarning("[Scene4] 현재 씬 저장 취소 — 빌드 중단"); return; }
            }

            // 1) 새 씬 (Camera + Light 기본 포함)
            var scene = UnityEditor.SceneManagement.EditorSceneManager.NewScene(
                UnityEditor.SceneManagement.NewSceneSetup.DefaultGameObjects,
                UnityEditor.SceneManagement.NewSceneMode.Single);

            // 2) LCC source (Virnect LccScene)
            var lccScene = AssetDatabase.LoadMainAssetAtPath(lccPath);
            if (lccScene == null) { Debug.LogError($"[Scene4] LccScene 로드 실패: {lccPath}"); return; }

            // 3) Aras-P GaussianSplatAsset
            var arasAsset = AssetDatabase.LoadMainAssetAtPath(asetPath);
            if (arasAsset == null) { Debug.LogError($"[Scene4] GaussianSplatAsset 없음: {asetPath}"); return; }
            ...
            if (lccRendererType == null) { Debug.LogError("[Scene4] LccSplatRenderer 타입 못 찾음"); return; }
            if (gsRendererType  == null) { Debug.LogError("[Scene4] GaussianSplatRenderer 타입 못 찾음"); return; }

            var bf = System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance;

            // 5) Splat_Virnect_1st_Cutter (좌측, X=-40) — 양쪽 동일 transform 사용 (identity)
            var virnectGO = new GameObject("Splat_Virnect_1st_Cutter");
            virnectGO.transform.position = new Vector3(-40f, 0f, 0f);
            virnectGO.transform.rotation = Quaternion.identity;
            virnectGO.AddComponent<MeshFilter>();
            virnectGO.AddComponent<MeshRenderer>();
            var lccComp = (MonoBehaviour)virnectGO.AddComponent(lccRendererType);
            lccComp.enabled = false;   // OnEnable 자동 호출 막기
            lccRendererType.GetField("scene", bf).SetValue(lccComp, lccScene);
            lccRendererType.GetField("lodLevel", bf).SetValue(lccComp, 4);   // LOD 4 (320K) — Edit mode 안전 / 비교 후 LOD 0 변경 가능
            lccComp.enabled = true;    // OnEnable → _TryLoad 트리거

            // 6) Splat_ArasP_1st_Cutter (우측, X=+40)
            var arasGO = new GameObject("Splat_ArasP_1st_Cutter");
            arasGO.transform.position = new Vector3(40f, 0f, 0f);
            arasGO.transform.rotation = Quaternion.identity;
            var gsComp = (MonoBehaviour)arasGO.AddComponent(gsRendererType);
            gsComp.enabled = false;
            var assetField = gsRendererType.GetField("m_Asset", bf) ?? gsRendererType.GetField("asset", bf);
            if (assetField != null) assetField.SetValue(gsComp, arasAsset);
            else { Debug.LogError("[Scene4] GaussianSplatRenderer.m_Asset 필드 못 찾음"); }
            gsComp.enabled = true;
            ...
            // 8) Directional Light — 살짝 기울이기
            var lightGO = GameObject.Find("Directional Light");
            if (lightGO != null) lightGO.transform.rotation = Quaternion.Euler(45f, -30f, 0f);

            // 9) Global Volume + SampleSceneProfile
            var volumeGO = new GameObject("__GlobalVolume");
            ...
            if (volumeType != null)
            {
                ...
                var profilePath = "Assets/Settings/SampleSceneProfile.asset";
                var profile = AssetDatabase.LoadMainAssetAtPath(profilePath);
                if (profile != null)
                {
                    var profileField = volumeType.GetField("sharedProfile", bf)
                                       ?? volumeType.GetField("m_Profile", bf);
                    var profileProp = volumeType.GetProperty("sharedProfile", bf)
                                      ?? volumeType.GetProperty("profile", bf);
                    if (profileProp != null) profileProp.SetValue(vol, profile);
                    else if (profileField != null) profileField.SetValue(vol, profile);
                }
                var isGlobalProp = volumeType.GetProperty("isGlobal", bf);
                if (isGlobalProp != null) isGlobalProp.SetValue(vol, true);
            }

            // 10) 라벨 텍스트 (TextMesh로 어느 쪽이 어느 SDK인지 표시)
            _AddLabel("Label_Virnect", new Vector3(-40f, 30f, 0f), "VIRNECT SDK\n(quad billboard)");
            _AddLabel("Label_Aras",    new Vector3( 40f, 30f, 0f), "ARAS-P\n(true 3DGS + SH)");

            // 11) 저장
            System.IO.Directory.CreateDirectory("Assets/Scenes");
            UnityEditor.SceneManagement.EditorSceneManager.SaveScene(scene, scenePath);
            Debug.Log($"[Scene4] 빌드 완료 → {scenePath}");
            Debug.Log("[Scene4] Scene/Game View에서 좌(Virnect) vs 우(Aras-P) 비교. WASD/마우스로 자유 이동 후 확인.");
        }

        static void _AddLabel(string name, Vector3 pos, string text)
        {
        ...
        [MenuItem("Tools/Lcc Drop Forge/Scene5 · Fix V-Bot lighting (light intensity + ambient + receiveShadows off)")]
        static void DevFixVBotLighting()
        {
            var sm = UnityEditor.SceneManagement.EditorSceneManager.GetActiveScene();

            // 1) Directional Light intensity ↑
            var lightGO = GameObject.Find("Directional Light");
            if (lightGO != null)
            {
                var l = lightGO.GetComponent<Light>();
                if (l != null) { Undo.RecordObject(l, "Bright light"); l.intensity = 2.5f; l.color = Color.white; EditorUtility.SetDirty(l); }
                Undo.RecordObject(lightGO.transform, "Light rotation");
                lightGO.transform.rotation = Quaternion.Euler(50f, -30f, 0f);
            }

            // 2) Ambient lighting (Environment Lighting Intensity)
            RenderSettings.ambientMode = UnityEngine.Rendering.AmbientMode.Trilight;
            RenderSettings.ambientSkyColor    = new Color(0.6f, 0.6f, 0.7f);
            ...
            // 3) V-Bot SkinnedMeshRenderer: receiveShadows = false (자기 그림자가 자기 가리는 것 방지)
            var player = System.Array.Find(sm.GetRootGameObjects(), r => r != null && r.name == "Player_VBot");
            int rendFix = 0;
            if (player != null)
            {
                foreach (var r in player.GetComponentsInChildren<Renderer>(true))
                {
                    Undo.RecordObject(r, "VBot lighting");
            ...
            UnityEditor.SceneManagement.EditorSceneManager.MarkSceneDirty(sm);
            Debug.Log($"[VBot Light] Directional intensity=2.5, ambient↑, {rendFix}개 Renderer receiveShadows=off · 캡처 다시 떠서 확인");

            // 캡처
            if (player != null)
            {
                var p = player.transform.position;
                var camPos = p + new Vector3(0f, 1.5f, -3f);
                _RenderView("Library/tmp_vbot_closeup.png", camPos, Quaternion.LookRotation(p + Vector3.up * 0.9f - camPos), 100f, 1280, 720, false, 1f);
            }
        }

        [MenuItem("Tools/Lcc Drop Forge/Scene5 · Brighten V-Bot materials (BaseColor=white) + capture")]
        static void DevBrightenVBotAndCapture()
        {
            const string vbotMatFolder = "Assets/Invector-3rdPersonController_LITE/3D Models/Characters/Invector@V-Bot 2.0/Material/Custom";
            var guids = AssetDatabase.FindAssets("t:Material", new[] { vbotMatFolder });
            int n = 0;
            foreach (var g in guids)
            {
                ...
                if (m == null) continue;
                Undo.RecordObject(m, "Brighten BaseColor");
            ...
            Debug.Log($"[VBot Bright] {n}개 머티리얼 _BaseColor → white");
        ...
        static void DevFixColliderProperAlign()
        {
            ...
            void FixOne(GameObject splat)
            {
                ...
                if (colTr != null)
                {
                    mc = colTr.GetComponent<MeshCollider>();
                    if (mc != null) srcMesh = mc.sharedMesh;
                }
                if (srcMesh == null)
                {
                    string lccName = splat.name.Substring("Splat_".Length);
                    var lcc = _FindLccSceneByName(lccName);
                    if (lcc == null) { Debug.LogWarning($"[ColliderFix v2] {splat.name}: LccScene 없음"); return; }
                    var plyPath = lcc.ResolveProxyMeshPlyAssetPath();
                    if (string.IsNullOrEmpty(plyPath)) return;
                    try { srcMesh = Virnect.Lcc.LccMeshPlyLoader.Load(System.IO.Path.GetFullPath(plyPath)); }
                    catch (System.Exception e) { Debug.LogError($"[ColliderFix v2] {splat.name} mesh load 실패: {e.Message}"); return; }
                    ...
                    mc = colGO.AddComponent<MeshCollider>();
                }

                // Z-up → Y-up 변환 (Aras-P PLY와 동일 좌표계)
                var converted = _ConvertZupToYupMesh(srcMesh, splat.name);
                mc.sharedMesh = converted;
                mc.enabled = true;
                mc.convex = false;

                // _ArasP child 와 동일하게 world identity 만들기 (부모 회전 inverse)
                Undo.RecordObject(colTr, "Fix collider rotation");
                ...
                EditorUtility.SetDirty(colTr);
                EditorUtility.SetDirty(mc);
                n++;
                Debug.Log($"[ColliderFix v2] {splat.name}: mesh Z-up→Y-up 변환 + world identity (Aras-P와 동일) · verts={converted.vertexCount:N0}");
            }
            foreach (var go in sm.GetRootGameObjects()) FixOne(go);
            var grp = System.Array.Find(sm.GetRootGameObjects(), r => r.name == "LccGroup");
            if (grp != null) foreach (Transform t in grp.transform) FixOne(t.gameObject);
            UnityEditor.SceneManagement.EditorSceneManager.MarkSceneDirty(sm);
            Debug.Log($"[ColliderFix v2] {n}개 콜라이더 정확 정렬 완료. Diag 메뉴로 검증");
        }
        ...
        [MenuItem("Tools/Lcc Drop Forge/Collider · FIX · force __LccCollider rotation to world Z-up→Y-up (270,0,0)")]
        static void DevFixColliderRotation()
        {
            var sm = UnityEditor.SceneManagement.EditorSceneManager.GetActiveScene();
            int n = 0;
            var worldZupToYup = Quaternion.Euler(270f, 0f, 0f);
            void FixOne(GameObject splat)
            {
                if (splat == null || !splat.name.StartsWith("Splat_") || splat.name.StartsWith("Splat_ArasP_")) return;
                var colTr = splat.transform.Find("__LccCollider");
                if (colTr == null) return;
                Undo.RecordObject(colTr, "Fix collider rotation");
                // 부모 회전과 무관하게 __LccCollider 자체 world rotation을 (270, 0, 0) 으로 강제
                colTr.localRotation = Quaternion.Inverse(splat.transform.rotation) * worldZupToYup;
                EditorUtility.SetDirty(colTr);
                n++;
                Debug.Log($"[ColliderFix] {splat.name}/__LccCollider localRotation 보정 → world={(splat.transform.rotation * colTr.localRotation).eulerAngles}");
            }
            foreach (var go in sm.GetRootGameObjects()) FixOne(go);
            var grp = System.Array.Find(sm.GetRootGameObjects(), r => r.name == "LccGroup");
            if (grp != null) foreach (Transform t in grp.transform) FixOne(t.gameObject);
            UnityEditor.SceneManagement.EditorSceneManager.MarkSceneDirty(sm);
            Debug.Log($"[ColliderFix] {n}개 __LccCollider 보정 완료. Diag 메뉴로 검증");
        }

        [MenuItem("Tools/Lcc Drop Forge/Scene5 · DIAG · dump V-Bot Renderer materials (read-only)")]
        static void DevDumpVBotRendererMaterials()
        {
            var sm = UnityEditor.SceneManagement.EditorSceneManager.GetActiveScene();
            var player = System.Array.Find(sm.GetRootGameObjects(), r => r != null && r.name == "Player_VBot");
            if (player == null) { Debug.LogError("[VBot Diag] Player_VBot 없음"); return; }
            int rendCount = 0;
            foreach (var r in player.GetComponentsInChildren<Renderer>(true))
            {
                rendCount++;
                var mats = r.sharedMaterials;
                Debug.Log($"[VBot Diag] {r.gameObject.name} ({r.GetType().Name}) — {mats.Length} slots:");
                for (int i = 0; i < mats.Length; i++)
                {
                    var m = mats[i];
                    if (m == null) Debug.Log($"  [{i}] NULL (magenta!)");
                    else Debug.Log($"  [{i}] '{m.name}' shader='{m.shader.name}' path={AssetDatabase.GetAssetPath(m)}");
                }
            }
            Debug.Log($"[VBot Diag] 총 {rendCount}개 Renderer 발견");
        }

        [MenuItem("Tools/Lcc Drop Forge/Scene5 · Force-reassign V-Bot materials in Player_VBot (fix textures)")]
        static void DevForceReassignVBotMaterials()
        {
            const string vbotMatFolder = "Assets/Invector-3rdPersonController_LITE/3D Models/Characters/Invector@V-Bot 2.0/Material/Custom";
            // 머티리얼 이름 → asset 매핑
            var matMap = new System.Collections.Generic.Dictionary<string, Material>();
            foreach (var g in AssetDatabase.FindAssets("t:Material", new[] { vbotMatFolder }))
            {
                var p = AssetDatabase.GUIDToAssetPath(g);
                var m = AssetDatabase.LoadAssetAtPath<Material>(p);
                if (m != null) matMap[m.name] = m;
            }
            Debug.Log($"[VBot Mat] V-Bot 머티리얼 asset {matMap.Count}개: {string.Join(", ", matMap.Keys)}");

            var sm = UnityEditor.SceneManagement.EditorSceneManager.GetActiveScene();
            var player = System.Array.Find(sm.GetRootGameObjects(), r => r != null && r.name == "Player_VBot");
            if (player == null) { Debug.LogError("[VBot Mat] Player_VBot 없음"); return; }

            int reassigned = 0, missing = 0;
            foreach (var rend in player.GetComponentsInChildren<Renderer>(true))
            {
                ...
                {
                    var current = mats[i];
                    string targetName = current != null ? current.name.Replace(" (Instance)", "") : null;
                    if (targetName != null && matMap.TryGetValue(targetName, out var assetMat))
                    {
                        if (current != assetMat) { mats[i] = assetMat; changed = true; reassigned++; Debug.Log($"[VBot Mat] {rend.gameObject.name}[{i}]: '{targetName}' → asset"); }
                    }
                    else if (current == null)
                    {
                        Debug.LogWarning($"[VBot Mat] {rend.gameObject.name}[{i}] material slot null"); missing++;
                    }
                    else
                    {
                        Debug.Log($"[VBot Mat] {rend.gameObject.name}[{i}] '{current.name}' (V-Bot 폴더 외 — skip)");
                    }
                }
                if (changed) { rend.sharedMaterials = mats; EditorUtility.SetDirty(rend); }
            }
            UnityEditor.SceneManagement.EditorSceneManager.MarkSceneDirty(sm);
            Debug.Log($"[VBot Mat] reassigned={reassigned}, null slots={missing}. ▶ Play 시 V-Bot 텍스처 확인");
        }

        [MenuItem("Tools/Lcc Drop Forge/Scene5 · Convert V-Bot materials to URP/Lit (fix magenta)")]
        static void DevConvertVBotMaterialsToURP()
        {
            const string vbotFolder = "Assets/Invector-3rdPersonController_LITE/3D Models/Characters/Invector@V-Bot 2.0/Material";
            var urpLit = Shader.Find("Universal Render Pipeline/Lit");
            if (urpLit == null) { Debug.LogError("[VBot URP] 'Universal Render Pipeline/Lit' shader 없음 — URP 패키지 확인"); return; }

            var guids = AssetDatabase.FindAssets("t:Material", new[] { vbotFolder });
            ...
            foreach (var g in guids)
            {
                ...
                if (m.shader == urpLit) { skipped++; continue; }

                // Standard → URP/Lit 슬롯 매핑 미리 캐시
                ...
                Texture bumpMap      = m.HasProperty("_BumpMap")       ? m.GetTexture("_BumpMap")       : null;
                float   bumpScale    = m.HasProperty("_BumpScale")     ? m.GetFloat("_BumpScale")       : 1f;
                Texture occlusionMap = m.HasProperty("_OcclusionMap")  ? m.GetTexture("_OcclusionMap")  : null;
                float   occlusionStr = m.HasProperty("_OcclusionStrength") ? m.GetFloat("_OcclusionStrength") : 1f;
                Texture emissionMap  = m.HasProperty("_EmissionMap")   ? m.GetTexture("_EmissionMap")   : null;
                Color   emissionCol  = m.HasProperty("_EmissionColor") ? m.GetColor("_EmissionColor")   : Color.black;

                // shader swap
                Undo.RecordObject(m, "Convert to URP/Lit");
                m.shader = urpLit;

                // URP/Lit 슬롯에 재할당
                if (m.HasProperty("_BaseMap"))      { m.SetTexture("_BaseMap", mainTex); m.SetTextureScale("_BaseMap", mainScale); m.SetTextureOffset("_BaseMap", mainOffset); }
                if (m.HasProperty("_BaseColor"))     m.SetColor("_BaseColor", color);
                if (m.HasProperty("_MetallicGlossMap")) m.SetTexture("_MetallicGlossMap", metallicMap);
                if (m.HasProperty("_Metallic"))      m.SetFloat("_Metallic", metallic);
                if (m.HasProperty("_Smoothness"))    m.SetFloat("_Smoothness", smoothness);
                if (m.HasProperty("_BumpMap"))       m.SetTexture("_BumpMap", bumpMap);
                if (m.HasProperty("_BumpScale"))     m.SetFloat("_BumpScale", bumpScale);
                if (m.HasProperty("_OcclusionMap"))  m.SetTexture("_OcclusionMap", occlusionMap);
                if (m.HasProperty("_OcclusionStrength")) m.SetFloat("_OcclusionStrength", occlusionStr);
                if (m.HasProperty("_EmissionMap"))   m.SetTexture("_EmissionMap", emissionMap);
                if (m.HasProperty("_EmissionColor")) m.SetColor("_EmissionColor", emissionCol);

                // URP keywords
                if (metallicMap != null) m.EnableKeyword("_METALLICSPECGLOSSMAP");
                if (bumpMap != null) m.EnableKeyword("_NORMALMAP");
                if (occlusionMap != null) m.EnableKeyword("_OCCLUSIONMAP");
                if (emissionMap != null || emissionCol.maxColorComponent > 0.01f)
                { m.EnableKeyword("_EMISSION"); m.globalIlluminationFlags = MaterialGlobalIlluminationFlags.RealtimeEmissive; }
                else m.DisableKeyword("_EMISSION");

                EditorUtility.SetDirty(m);
                converted++;
                Debug.Log($"[VBot URP] {m.name}.mat → URP/Lit (mainTex={mainTex?.name})");
            }
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
            Debug.Log($"[VBot URP] 변환 완료 · converted={converted}, skipped={skipped} (이미 URP)");
        }
        ...
        // [기존 v2 메뉴 코드 — 더 이상 사용 안 함. v3 위에서 return으로 종료. dead code 보존]
        static void _DeadCode_Scene5WalkableV2()
        {
            var sm = UnityEditor.SceneManagement.EditorSceneManager.GetActiveScene();
            var roots = sm.GetRootGameObjects();
            ...
            GameObject group;
            if (existingGroup != null) group = existingGroup;
            else
            {
                group = new GameObject("LccGroup");
                Undo.RegisterCreatedObjectUndo(group, "Create LccGroup parent");
            ...
            foreach (var s in splats)
            {
                if (s.transform.parent != group.transform)
                {
                    Undo.SetTransformParent(s.transform, group.transform, "Reparent to LccGroup");
                }
            }

            // 1.6) 자식 Splat 들의 모든 collider 비활성/제거 — LccGroup 단일 MeshCollider 외엔 없게
            int disabledCount = 0;
            foreach (var s in splats)
            {
                foreach (var col in s.GetComponentsInChildren<Collider>(true))
                {
                    if (col == null) continue;
                    Undo.RecordObject(col, "Disable child collider");
                    col.enabled = false;
                    disabledCount++;
                }
            }
            // 기존 별도 root __CombinedCollider 있으면 제거
            var oldCC = System.Array.Find(roots, r => r != null && r.name == "__CombinedCollider");
            if (oldCC != null) Undo.DestroyObjectImmediate(oldCC);
            Debug.Log($"[Walkable] 자식 collider {disabledCount}개 disabled · 기존 __CombinedCollider 제거");

            // 2) 각 Splat 의 proxy mesh → CombineMeshes (world-space)
            //    __LccCollider 자식 있으면 그 sharedMesh 사용, 없으면 LccScene asset에서 직접 로드
            ...
            foreach (var s in splats)
            {
                ...
                {
                    var mc = colTr.GetComponent<MeshCollider>();
                    if (mc != null && mc.sharedMesh != null) { mesh = mc.sharedMesh; mat = colTr.localToWorldMatrix; }
                }

                if (mesh == null)
                {
                    // 자식 없거나 비어있으면 LccScene asset 에서 proxy PLY 직접 로드
                    string lccName = s.name.Substring("Splat_".Length);
                    var lcc = _FindLccSceneByName(lccName);
                    if (lcc != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(plyPath) && System.IO.File.Exists(System.IO.Path.GetFullPath(plyPath)))
                        {
                            ...
                            catch (System.Exception e) { Debug.LogWarning($"[Walkable] {s.name} mesh load 실패: {e.Message}"); }
                        }
                    }
                    mat = s.transform.localToWorldMatrix;   // 부모 transform 적용
                }

                if (mesh == null) { Debug.LogWarning($"[Walkable] {s.name}: proxy mesh 못 가져옴 — skip"); continue; }
                combineList.Add(new CombineInstance { mesh = mesh, transform = mat });
            }
            if (combineList.Count == 0) { Debug.LogError("[Walkable] proxy mesh 없음 — Ensure MeshColliders 먼저 실행하거나 LccScene asset 확인"); return; }

            // 3) 합쳐진 mesh
            var combined = new Mesh
            {
                name = "Scene5_CombinedCollider",
                indexFormat = UnityEngine.Rendering.IndexFormat.UInt32
            };
            combined.CombineMeshes(combineList.ToArray(), true, true);
            Debug.Log($"[Walkable] CombinedMesh {combined.vertexCount:N0} verts ({combineList.Count} splats merged)");

            // 4) LccGroup 부모에 단일 MeshCollider 부착 (combined mesh — LccGroup local space = world space, identity transform)
            var combinedMC = group.GetComponent<MeshCollider>();
            if (combinedMC == null) combinedMC = group.AddComponent<MeshCollider>();
            combinedMC.sharedMesh = combined;
            combinedMC.convex = false;

            // 6) Spawn Player (CharacterController + LccPlayerController)
            var existingPlayer = System.Array.Find(roots, r => r != null && r.name == "Player_LccWalker");
            if (existingPlayer != null) Undo.DestroyObjectImmediate(existingPlayer);

            var b = combinedMC.bounds;
            var spawnPos = new Vector3(b.center.x, b.max.y + 5f, b.center.z);

            var player = new GameObject("Player_LccWalker");
            ...
            // visible capsule (카메라가 어디 있는지 보이게)
            ...
            var ctrlType = System.Type.GetType("LccPlayerController, Assembly-CSharp");
            if (ctrlType != null) player.AddComponent(ctrlType);
            else Debug.LogWarning("[Walkable] LccPlayerController 타입 없음 (컴파일 대기 후 다시 실행)");

            // 7) Camera follow
            var camGO = GameObject.Find("Main Camera");
            if (camGO == null) { camGO = new GameObject("Main Camera"); camGO.AddComponent<Camera>(); camGO.tag = "MainCamera"; }
            camGO.transform.position = spawnPos + new Vector3(0f, 1f, -5f);
            camGO.transform.LookAt(spawnPos + Vector3.up * 1.5f);
            var cam = camGO.GetComponent<Camera>();
            if (cam != null) { cam.farClipPlane = 5000f; cam.nearClipPlane = 0.1f; cam.fieldOfView = 70f; }

            var followType = System.Type.GetType("LccCameraFollow, Assembly-CSharp");
            if (followType != null)
            {
                var follow = camGO.GetComponent(followType) as MonoBehaviour;
                if (follow == null) follow = camGO.AddComponent(followType) as MonoBehaviour;
                followType.GetField("target")?.SetValue(follow, player.transform);
            }
            else Debug.LogWarning("[Walkable] LccCameraFollow 타입 없음");

            // 8) PlayerController.cameraTr 연결 (reflection)
            if (ctrlType != null)
            {
                var pc = player.GetComponent(ctrlType);
                if (pc != null) ctrlType.GetField("cameraTr")?.SetValue(pc, camGO.transform);
            }
